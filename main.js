...
function updatePeersList() {
  // Update the connected peer name in the collapsed header
  if (isHost) {
    const peerEntries = Object.entries(peerUsernames).filter(([pid]) => pid !== myPeerId);
    if (peerEntries.length > 0) {
      connectedPeerName.textContent = `${peerEntries.length} peer${peerEntries.length !== 1 ? 's' : ''}`;
    } else {
...
function setupConnHandlers(conn, pid, isIncoming) {
  ...
  conn.on("open", () => {
    ...
    // Update the connected peer name display
    if (pid !== myPeerId) {
      connectedPeerName.textContent = peerUsernames[pid] || assignDefaultUsernameToPeer(pid);
    }
  ...
  conn.on('data', async data => {
    ...
    if (data && data.type === 'file-request' && data.fileMsgId) {
      ...
      while (offset < file.size && !canceled) {
        ...
        const chunk = await slice.arrayBuffer(); // Read chunk as ArrayBuffer

        conn.send({ type: 'file-chunk', data: chunk, chunkId, fileMsgId }); // Send the chunk
        awaitingAcks++;
        ...
        ackTimeouts[chunkId] = setTimeout(function retryChunk() {
          if (!chunkAckMap[chunkId] && !canceled) { // If not acknowledged and not canceled
            ...
            if (retriesMap[chunkId] > maxRetries) {
              ...
              conn.send({ type: 'file-chunk', data: chunk, chunkId, fileMsgId }); // Resend
...
async function startFileDownload(tile, fileMsgId) {
  ...
  conn.on('data', async chunk => {
    ...
    if (chunk.type === 'file-chunk' && chunk.data) {
      try {
        await fileWriter.write(new Uint8Array(chunk.data)); // Write chunk to file
